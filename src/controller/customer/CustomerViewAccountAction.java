...
public class CustomerViewAccountAction extends Action {
	...
	public String perform(HttpServletRequest request) {
		...
		try {
			...
			Util.i("customerId = ", customer.getId(), ", currentAmount = ", currentAmount, ", pendingAmount = ",
					pendingAmount, ", validAmount = ", validAmount);
			...
			Position[] positionList = positionDAO.readUserPosition(customer.getId());
			...
			for (int i = 0; i < positionList.length; i++) {
				...
				if (price != null) {
					shareList[i].setAmount(price.getPriceValue() * positionList[i].getShareValue());
