...
public class ViewTransactionHistoryAction extends Action {
	...
	public String perform(HttpServletRequest request) {
		...
		try {
			...
			TransactionBean[] transactionBeans = transactionDAO.readByCustomerId(customer.getId());
			...
			for (int i = 0; i < transactionBeans.length; i++) {
				...
				if (transactionBeans[i].getShare() != 0) {
					transactions[i].setShare(transactionBeans[i].getShareValue());
				}
				if (transactionBeans[i].getPrice() != 0) {
					transactions[i].setPrice(transactionBeans[i].getPriceValue());
				}
				if (transactionBeans[i].getAmount() != 0) {
					transactions[i].setAmount(transactionBeans[i].getAmountValue());
				}
				transactions[i].setType(transactionBeans[i].getTypeDescription());
			}
