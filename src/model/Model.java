...
public class Model {
	...
	public double[] getShare(int customerId, int fundId) throws RollbackException {
		synchronized (amountLock) {
			...
			double currentShare = position == null ? 0 : position.getShareValue();
	...
	public void commitTransaction(TransactionBean transaction) throws RollbackException {
		synchronized (amountLock) {
			...
			case TransactionBean.REQUEST_CHECK:
				...
				if (validAmount < transaction.getAmountValue()) {
					throw new RollbackException(Constants.THERE_IS_NOT_ENOUGH_MONEY);
				}
				transactionDAO.create(transaction);
				break;

			case TransactionBean.SELL_FUND:
				Position position = positionDAO.read(transaction.getCustomerId(), transaction.getFundId());
				if (position == null) {
					throw new RollbackException("illegal fund id");
				}
				double currentShare = position.getShareValue();
				double pendingShare = transactionDAO.getPendingShare(transaction.getCustomerId(),
	...
	public void processTransaction(TransactionBean transactionBean, long newDate) throws RollbackException {
		synchronized (amountLock) {
	...
	private void processBuy(long date, TransactionBean transactionBean) throws RollbackException {
		Transaction.begin();
		Price price = priceDAO.getCurrentPrice(transactionBean.getFundId());
		double currentPrice = price == null ? Constants.MIN_VALUE_OF_PRICE_INPUT : price.getPriceValue();
		Customer customer = customerDAO.read(transactionBean.getCustomerId());
		Position position = positionDAO.read(transactionBean.getCustomerId(), transactionBean.getFundId());
		if (position == null) {
		...
		double incrementOfShare = transactionBean.getAmountValue() / currentPrice;
		...
		Util.i("amount: ", oldAmount, " - ", decrementOfAmount, " = ", newAmount);
	...
	private void processSell(long date, TransactionBean transactionBean) throws RollbackException {
		Transaction.begin();
		Price price = priceDAO.getCurrentPrice(transactionBean.getFundId());
		double currentPrice = price == null ? Constants.MIN_VALUE_OF_PRICE_INPUT : price.getPriceValue();
		Position position = positionDAO.read(transactionBean.getCustomerId(), transactionBean.getFundId());
		...
		long oldAmount = customer.getAmount();
		long incrementOfAmount = (long) (Util.truncate(decrementOfShare * currentPrice, 100) * 100);
		long newAmount = oldAmount + incrementOfAmount;
		Util.i("amount: ", oldAmount, " + ", incrementOfAmount, " = ", newAmount);
	...
	private void processRequest(long date, TransactionBean transactionBean) throws RollbackException {
		Transaction.begin();
		Customer customer = customerDAO.read(transactionBean.getCustomerId());
	...
	private void processDeposit(long date, TransactionBean transactionBean) throws RollbackException {
		Transaction.begin();
		Customer customer = customerDAO.read(transactionBean.getCustomerId());
