				...
				<c:forEach var="fund" items="${funds}">
					<tr>
						...
						<td><input type="hidden" name="ids"
							value="${fund.getFundId()}" /><input type="text" name="prices"
							value="${fund.getPrice()}" class="form-control" /></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
		<p style="color: grey">*Please enter numbers with no more than 2
			digits after the decimal</p>
		<br> <br>
		<div class="form-group">
			<label for="exampleInputEmail1">New transition day</label><input
				type="date" class="form-control" name="date"
				min="${minNewTransactionDay }" max="2050-01-01"
				value="${minNewTransactionDay }">
		</div>
